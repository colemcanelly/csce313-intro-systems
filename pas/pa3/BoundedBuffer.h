...
private:
	...
	std::mutex buffer_mutex;						// mutex
	// 2 cv's:
	std::condition_variable not_empty;	// one for checking not empty 	(data is available)
	std::condition_variable not_full;	// one for checking not full	(slot is available)
