...
int main (int argc, char* argv[]) {
	...
	while (true) {
		int new_fd = control_channel->accept_conn();

		cout << "New process thread :: ";
		thread t = thread{handle_process_loop, new TCPRequestChannel{new_fd} };

		// somehow (detach or join) reap the threads.
		t.join();
		cout << "Done" << endl;
	}

	// MESSAGE_TYPE q = QUIT_MSG;
	// control_channel->cwrite(&q, sizeof(MESSAGE_TYPE));
