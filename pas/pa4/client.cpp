...
int main (int argc, char* argv[]) {
    ...
    cout << "\tWorker threads" << endl;
    cout << "\t\tw:0";
    vector<worker*> worker_threads;
    for (int i = 1; i <= w; ++i)
    {
        cout << "\r\t\tw:" << i;
        worker_threads.push_back(new worker{&request_buffer, &response_buffer, a, r, m});
    }

    // CONSUMES RESPONSES :: histogram threads
    cout << "\tHistogram threads" << endl;
...
void file_thread_helper ( void* p_dest, string& __fname, int64_t __offset, int __length )
{
    ...
    if (!(__offset || __length)) {       // FIFORequestChannel
        ((TCPRequestChannel*)p_dest)->cwrite(buffer, msgLen);
    } else {                        // Bounded buffer
